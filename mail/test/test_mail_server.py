...
context.load_verify_locations('ssl/mail.pem')
...
context_no_host_check.load_verify_locations('ssl/mail.pem')
context_no_host_check.check_hostname = False 


def test_imap_ssl_certificate():
    with socket.create_connection((container_ip, 1993)) as sock:
        with context.wrap_socket(sock, server_hostname=hostname) as ssock:
            assert (('commonName', hostname),) in ssock.getpeercert()['subject']

def test_smtp_ssl_certificate():
    with socket.create_connection((container_ip, 1465)) as sock:
        with context.wrap_socket(sock, server_hostname=hostname) as ssock:
            assert (('commonName', hostname),) in ssock.getpeercert()['subject']

def test_imap_starttls_certificate():
    with imaplib.IMAP4(container_ip, port=1143) as imap:
        imap.starttls(ssl_context=context_no_host_check) # TODO: host check

def test_smtp_starttls_certificate():
    with smtplib.SMTP(container_ip, port=1025) as smtp:
...
def test_smtp_msa_starttls_certificate():
    with smtplib.SMTP(container_ip, port=1587) as smtp:
        smtp.starttls(context=context_no_host_check) # TODO: host check

def test_smtp_local_delivery_allowed():
    with smtplib.SMTP(container_ip, local_hostname='example.com', port=1025) as smtp:
        smtp.sendmail('test@example.com', 'gert@gertvv.nl',
            'From: test@example.com\nTo: gert@gertvv.nl\nSubject: testing 123\n\nTesting!')

def test_smtp_relay_forbidden():
    with smtplib.SMTP(container_ip, local_hostname='gertvv.nl', port=1025) as smtp:
...
def test_smtp_auth_invalid_credentials():
    with smtplib.SMTP_SSL(container_ip, port=1465, context=context_no_host_check) as smtp:
...
def test_imaps_list_messages():
    with imaplib.IMAP4_SSL(container_ip, port=1993, ssl_context=context_no_host_check) as imap:
